...
Result<void> ValidateMessage(const Json& json) {
  ...
  auto from_id = message["from"]["id"].get<int64_t>();
  auto chat_id = message["chat"]["id"].get<int64_t>();
  if (from_id != USER_ID or chat_id != USER_ID) {
    return tl::make_unexpected(make_error_code(BotError::kUserIsNotAllowed));
  }
  return {};
}

ResultRef<JsonHandler> GetJsonHandler(
    const Json& json, const JsonValidatorHandler::HandlersStorage& handlers) {
  ...
  auto prefix = message.substr(first_entity["offset"].get<int>() + 1,
...
tl::expected<JsonHandler*, Json> JsonValidatorHandler::ValidateAndGetHandler(
